...
public class Jail {
	...
	public void load() {
		File f = new File(SwornJail.p.getDataFolder(), "jail.txt");
		...
		if (!f.exists()) {
			...
			if (f.exists()) {
				...
				this.setMin(SwornJail.p.getConfig().getVector("jailregion.jailmin"));
				this.setMax(SwornJail.p.getConfig().getVector("jailregion.jailmax"));
				World w = Bukkit.getWorld(SwornJail.p.getConfig().getString("jailregion.jailworld"));
				this.setWorld((w != null) ? w : Bukkit.getWorlds().get(0));
				Vector spawn = SwornJail.p.getConfig().getVector("jailregion.jailspawn");
				this.setSpawn(new Location(w, spawn.getX(), spawn.getY(), spawn.getZ(),
						SwornJail.p.getConfig().getInt("jailregion.jailspawnyaw"), 0F));
				Vector exit = SwornJail.p.getConfig().getVector("jailregion.jailexit");
				this.setExit(new Location(w, exit.getX(), exit.getY(), exit.getZ(),
						SwornJail.p.getConfig().getInt("jailregion.jailexityaw"), 0F));
